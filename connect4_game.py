...
WINDOW_WIDTH, WINDOW_HEIGHT = WIDTH * CELL_SIZE + 2 * CELL_SIZE, (HEIGHT + 2.5) * CELL_SIZE + 2
    ...
    for col in range(WIDTH):
        for row in range(HEIGHT):
            pygame.draw.rect(
                screen,
                BACKGROUND_COLOR,
                (board_rect.left + col * CELL_SIZE, board_rect.top + (row + 1.5) * CELL_SIZE, CELL_SIZE, CELL_SIZE),
            )
            pygame.draw.circle(
                screen,
                GRID_COLOR,
...
def main():
    ...
    while True:
        ...
        ((WINDOW_WIDTH - WIDTH * CELL_SIZE) // 2 + WIDTH * CELL_SIZE // 2, CELL_SIZE // 2 + 25), 
        CELL_SIZE // 2 - 5,
    )
...
def end_screen(message, screen, player1_wins, player2_wins, draws):
    ...
    text_large_surface.blit(text_large_black, (2, 2))  # Offset by (2, 2) to create a border

    # Blit the white text on top of the black-bordered text
    text_large_surface.blit(text_large_white, (0, 0))

    rect_large = text_large_surface.get_rect(center=(WIDTH * CELL_SIZE // 2, HEIGHT * CELL_SIZE // 2))
    ...
    stats_text_surface.blit(stats_text_black, (2, 2))  # Offset by (2, 2) to create a border
    ...
    instructions_white = font_small.render("Press any key to continue", True, (255, 255, 255))

    # Create a surface with an alpha channel for instructions
    instructions_surface = pygame.Surface(instructions_white.get_size(), pygame.SRCALPHA)
    ...
    instructions_surface.blit(instructions_black, (2, 2))  # Offset by (2, 2) to create a border
    ...
    padding = 20
    background_width = max(rect_large.width, stats_text_surface.get_width(), instructions_surface.get_width()) + 2 * padding
    background_height = rect_large.height + stats_text_surface.get_height() + instructions_surface.get_height() + 4 * padding
    ...
    background_surface.blit(text_large_surface, (background_width // 2 - rect_large.width // 2, padding))

    # Blit the game statistics on the background surface
    background_surface.blit(stats_text_surface, (padding, rect_large.height + 2 * padding))
    ...
    background_surface.blit(instructions_surface, (instructions_x, rect_large.height + stats_text_surface.get_height() + 3 * padding))

    # Adjust the position of the background_rect to center it on the screen
    background_rect = background_surface.get_rect(center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2))
