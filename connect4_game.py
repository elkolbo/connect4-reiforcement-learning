...
def end_screen(message, screen, player1_wins, player2_wins, draws):
    ...
    rect_large = text_large.get_rect(
    ...
    padding = 20
    background_width = (
        max(rect_large.width, stats_text.get_width(), instructions.get_width())
        + 2 * padding
    )
    background_height = (
        rect_large.height
        + stats_text.get_height()
    ...
    background_surface.blit(
        text_large, (background_width // 2 - rect_large.width // 2, padding)
    )
    ...
    background_surface.blit(
        instructions,
        (instructions_x, rect_large.height + stats_text.get_height() + 3 * padding),
    )
